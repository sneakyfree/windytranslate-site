...
const LANGCHECK = process.env.LANGCHECK || path.join(__dirname, 'langcheck_results.jsonl');

// ISO 639-5 collective codes. These name language FAMILIES, and the models
// targeting them are OPUS group models that legitimately pick a member — so a
// language-identity check can never "match" them and must not mark them wrong.
const FAMILY_CODES = new Set(['aav', 'dra', 'iir', 'inc', 'trk', 'tut', 'zle', 'jap',
  'roa', 'gem', 'sla', 'ine', 'itc', 'cel', 'bat', 'fiu', 'afa', 'nic', 'bnt',
  'cus', 'sem', 'map', 'poz', 'phi', 'tai', 'sit', 'mkh', 'urj', 'trc', 'alv',
  'euq', 'cpf', 'cpp', 'crp', 'art', 'mul', 'zls', 'zlw', 'zle', 'ber', 'cau']);
...
function main() {
  ...
  // Language-identity results for pairs FLORES-200 cannot score. This is a
  // different KIND of evidence — it says the model writes the right language,
  // not that it writes it well — so it is reported separately and never mixed
  // into the chrF++ bands.
  let verified = [];
  if (fs.existsSync(LANGCHECK)) {
    const lc = fs.readFileSync(LANGCHECK, 'utf8')
      .split('\n').filter(Boolean).map((l) => JSON.parse(l));
    verified = lc
      .filter((r) => (r.verdict === 'MATCH' || r.verdict === 'SCRIPT-OK')
        && !FAMILY_CODES.has(r.tgt))
      .map((r) => ({
        id: r.model,
        repo: `WindyTranslate/${r.model}`,
        src: r.src,
      ...
      .sort((a, b) => a.tgtName.localeCompare(b.tgtName));
  }
  ...
  const out = {
    ...
    languageVerified: {
      ...
      method: 'GlotLID language identification on model output — confirms the model writes the language it claims. Not a quality score.',
      models: verified,
    },
    models,
  };
  ...
  console.log(`  ${verified.length} of those independently language-verified with GlotLID`);
  if (unnamed.size) {
